#!/usr/bin/env bash
# Installs cldLAB (Claude Control Plane) as a systemd user service on
# Ubuntu/Debian hosts. Run as the user who will own the service (not root).
#
# Usage:
#   ./install.sh                          # auto-detects project root (two levels up)
#   ./install.sh --project /path/to/repo  # explicit project root
#   ./install.sh --port 6002              # override port (default: .port file, else 5080)
...
TEMPLATE="$SCRIPT_DIR/cldlab.service.template"

# ── Uninstall ────────────────────────────────────────────────────────────────
if [[ "$UNINSTALL" -eq 1 ]]; then
  echo "Uninstalling cldlab service..."
...
# ── Resolve port (most specific wins): --port ARG > .port file > 5080 ─────────
if [[ -z "$PORT" ]]; then
  if [[ -f "$PROJECT_PATH/.port" ]]; then
    PORT="$(tr -d '[:space:]' < "$PROJECT_PATH/.port")"
  fi
fi
[[ -z "$PORT" ]] && PORT="5080"

if ! [[ "$PORT" =~ ^[0-9]+$ ]]; then
  echo "ERROR: resolved port '$PORT' is not a number."
  echo "  Fix $PROJECT_PATH/.port or pass --port N."
  exit 1
fi
...
# ── Install ──────────────────────────────────────────────────────────────────
echo "Installing cldlab systemd user service"
